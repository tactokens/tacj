...
import com.tacplatform.tacj.matcher.*;
import com.tacplatform.tacj.transactions.*;

import java.util.Collection;
import java.util.List;

import static com.tacplatform.tacj.Asset.isTac;
...
public class Transactions {
    ...
    public static ReissueTransactionV2 makeReissueTx(PrivateKeyAccount sender, byte chainId, String assetId, long quantity,
        ...
        if (isTac(assetId)) {
            throw new IllegalArgumentException("Cannot reissue TAC");
        }
    ...
    public static BurnTransactionV2 makeBurnTx(PrivateKeyAccount sender, byte chainId, String assetId, long amount, long fee, long timestamp) {
        if (isTac(assetId)) {
            throw new IllegalArgumentException("Cannot burn TAC");
        }
    ...
    public static SponsorTransaction makeSponsorTx(PrivateKeyAccount sender, String assetId, long minAssetFee, long fee, long timestamp) {
        if (isTac(assetId)) {
            throw new IllegalArgumentException("Cannot sponsor TAC");
        }
        if (minAssetFee < 0) {
    ...
    public static Order makeOrder(PrivateKeyAccount account, String matcherKey, Order.Type orderType,
        ...
        return new OrderV3(account, new PublicKeyAccount(matcherKey, account.getChainId()), orderType, assetPair, amount, price, timestamp,
                expiration, matcherFee, Asset.TAC, Order.V3);
    }
