...
import com.tacplatform.tacj.matcher.CancelOrder;
import com.tacplatform.tacj.matcher.DeleteOrder;
...
public class Node {
    private static final String DEFAULT_NODE = "https://nodes.tactoken.io";
    ...
    private final TacJsonMapper tacJsonMapper;
    ...
    public Node() {
        try {
            ...
            this.tacJsonMapper = new TacJsonMapper((byte) 'T');
    ...
    public Node(String uri, char chainId) throws URISyntaxException {
        ...
        this.tacJsonMapper = new TacJsonMapper((byte) chainId);
        this.client = createDefaultClient();
    ...
    public Node(String uri, byte chainId) throws URISyntaxException {
        this.uri = new URI(uri);
        this.tacJsonMapper = new TacJsonMapper(chainId);
        this.client = createDefaultClient();
    ...
    public BalanceDetails getBalanceDetails(String address) throws IOException {
        return tacJsonMapper.convertValue(send("/addresses/balance/details/" + address), BALANCE_DETAILS);
    }
    ...
    public List<AssetDetails> getNFTs(String address, int limit) throws IOException {
        return tacJsonMapper.convertValue(send("/assets/nft/" + address + "/limit/" + limit), ASSET_DETAILS_LIST);
    }

    public List<AssetBalance> getAssetsBalance(String address) throws IOException {
        return tacJsonMapper.convertValue(send("/assets/balance/" + address, "balances"), ASSET_BALANCE_LIST);
    }

    public List<DataEntry> getData(String address) throws IOException{
        return tacJsonMapper.convertValue(send(String.format("/addresses/data/%s", address)),ADDRESS_DATA);
    }

    public DataEntry getDataByKey(String address, String key) throws IOException{
        return tacJsonMapper.convertValue(send(String.format("/addresses/data/%s/%s", address, key)),ADDRESS_DATA_BY_KEY);
    }

    public AssetDetails getAssetDetails(String assetId) throws IOException {
        return tacJsonMapper.convertValue(send("/assets/details/" + assetId), AssetDetails.class);
    }

    public List<LeaseInfo> getActiveLeases(String address) throws IOException {
        return tacJsonMapper.convertValue(send("/leasing/active/" + address), LEASE_INFO_LIST);
    }
    ...
    public Transaction getTransaction(String txId) throws IOException {
        return tacJsonMapper.convertValue(send("/transactions/info/" + txId), Transaction.class);
    }

    public Map<String, Object> getTransactionData(String txId) throws IOException {
        return tacJsonMapper.convertValue(send("/transactions/info/" + txId), TX_INFO);
    }
    ...
    public List<Transaction> getAddressTransactions(String address, int limit, String after) throws IOException {
        ...
        return tacJsonMapper.<List<List<Transaction>>>convertValue(send(requestUrl),new TypeReference<List<List<Transaction>>>(){}).get(0);
    }
    ...
    public Block getBlock(int height) throws IOException {
        return tacJsonMapper.convertValue(send("/blocks/at/" + height), Block.class);
    }
    ...
    public BlockHeader getBlockHeader(int height) throws IOException {
        return tacJsonMapper.convertValue(send("/blocks/headers/at/" + height), BlockHeader.class);
    }
    ...
    public BlockHeader getLastBlockHeader() throws IOException {
        return tacJsonMapper.convertValue(send("/blocks/headers/last"), BlockHeader.class);
    }
    ...
    public Block getBlock(String signature) throws IOException {
        return tacJsonMapper.convertValue(send("/blocks/signature/" + signature), Block.class);
    }
    ...
    public List<String> getAliasbyAddress(String address) throws IOException {
        return tacJsonMapper.convertValue(send("/alias/by-address/" + address), new TypeReference<List<String>>() {});
    }
    ...
    public Rewards getBlockchainRewards() throws IOException {
        return tacJsonMapper.convertValue(send("/blockchain/rewards"), Rewards.class);
    }
    ...
    public Rewards getBlockchainRewards(int height) throws IOException {
        return tacJsonMapper.convertValue(send(String.format("/blockchain/rewards/%d", height)), Rewards.class);
    }
    ...
    public Order createOrder(PrivateKeyAccount account, String matcherKey, AssetPair assetPair, Order.Type orderType,
        ...
        return tacJsonMapper.treeToValue(tree.get("message"), Order.class);
    }
    ...
    private HttpUriRequest request(ApiJson obj) throws JsonProcessingException {
        ...
        request.setEntity(new StringEntity(tacJsonMapper.writeValueAsString(obj), ContentType.APPLICATION_JSON));
        return request;
    }
    ...
    private <T> T parse(HttpResponse r, TypeReference<T> ref) throws IOException {
        return tacJsonMapper.readValue(r.getEntity().getContent(), ref);
    }

    private JsonNode parse(HttpResponse r, String... keys) throws IOException {
        JsonNode tree = tacJsonMapper.readTree(r.getEntity().getContent());
        for (String key : keys) {
            tree = tree.get(key);
        }
        return tree;
